<table>
    ...
    <tr class="item-row" data-key="<%=k %>">
        ...
        <td class="info item-play<%= Settings.seriesUITransparency ? '' : ' transpOff' %>"><%=torrent.quality || '-' %></td>
        <td class="info item-play<%= Settings.seriesUITransparency ? '' : ' transpOff' %>"><%=torrent.filesize || torrent.size || '-' %></td>
