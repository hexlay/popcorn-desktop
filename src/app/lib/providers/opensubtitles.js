...
(function (App) {
    ...
    OpenSubtitles.prototype.fetch = function (queryParams) {
        return new Promise(function (resolve, reject) {
            ...
            if (queryParams.imdbid) {
                searchQuery.imdb_id = String(queryParams.imdbid).replace(/^tt/, '');
            }
    ...
    OpenSubtitles.prototype.getDownloadUrl = function (fileId) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var downloadPromise;
            const savedToken = AdvSettings.get('opensubtitlesToken');
            if (!authToken && !savedToken) {
                ...
                if (username && password) {
                    ...
                    return reject(new Error('OpenSubtitles login is required to download subtitles'));
                }
            } else {
                downloadPromise = client.download({
                    file_id: parseInt(fileId, 10),
                    token: authToken || savedToken
                });
            }
